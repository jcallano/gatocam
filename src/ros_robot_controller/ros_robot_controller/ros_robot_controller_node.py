...
from ros_robot_controller_msgs.msg import ButtonState, BuzzerState, LedState, MotorsState, BusServoState, SetBusServoState, ServosPosition, SetPWMServoState, Sbus, OLEDState, PWMServoState
...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        ...
        self.declare_parameter('reset_dtr_rts', True)
        self.declare_parameter('reset_pulse_ms', 100)
        self.declare_parameter('reset_post_ms', 500)

        port = self.get_parameter('port').value
        baudrate = self.get_parameter('baudrate').value
        timeout = self.get_parameter('timeout').value
        reset_dtr_rts = self.get_parameter('reset_dtr_rts').value
        reset_pulse_ms = self.get_parameter('reset_pulse_ms').value
        reset_post_ms = self.get_parameter('reset_post_ms').value

        self.get_logger().info(
            f'Using serial port={port} baudrate={baudrate} timeout={timeout} '
            f'reset_dtr_rts={reset_dtr_rts} reset_pulse_ms={reset_pulse_ms} reset_post_ms={reset_post_ms}'
        )

        self.board = Board(device=port, baudrate=baudrate, timeout=timeout)
        if reset_dtr_rts:
            try:
                self.board.port.dtr = True
                self.board.port.rts = True
                time.sleep(reset_pulse_ms / 1000.0)
                self.board.port.dtr = False
                self.board.port.rts = False
                time.sleep(reset_post_ms / 1000.0)
            except Exception as exc:
                self.get_logger().warn(f'Failed to toggle DTR/RTS reset: {exc}')
